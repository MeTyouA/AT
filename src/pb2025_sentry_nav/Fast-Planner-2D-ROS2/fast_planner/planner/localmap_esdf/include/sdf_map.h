...
private:
  ...
  std::vector<double> occupancy_buffer, occupancy_buffer_neg, distance_buffer,
      distance_buffer_neg, distance_buffer_all, tmp_buffer1, tmp_buffer2;
...
public:
  ...
  void updateESDF3d();
  void getSliceESDF(const double height, const double res,
                    Eigen::Vector4d range, vector<Eigen::Vector3d> &slice,
  ...
  bool fillLocalMinima(const vector<Eigen::Vector3d> &peaks,
                       Eigen::Vector3d &center, Eigen::Vector3d &scale);

  bool tryFillMinima(const Eigen::Vector3d &pt, Eigen::Vector3d &center,
                     Eigen::Vector3d &cube_len);

  void initMap(double x_size, double y_size, double z_size, double resolution_,
               Eigen::Vector3d org, double inflate_values);
  ...
  Eigen::Vector3d getMapSize() const { return map_size; }
  Eigen::Vector3i getGridSize() const { return grid_size; }
