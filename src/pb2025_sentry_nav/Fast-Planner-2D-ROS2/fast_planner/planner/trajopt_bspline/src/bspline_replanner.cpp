...
void BsplineReplanner::configureEsdfPenalty(double distance_weight, double safe_distance) {
  if (traj_optimizer) {
    traj_optimizer->setEsdfPenalty(distance_weight, safe_distance);
  }
}

void BsplineReplanner::configureRcEsdf(
  ...
  if (traj_optimizer) {
    traj_optimizer->setRcEsdfConfig(
        enabled, lambda, margin, map_width, map_height, resolution, query_radius,
        max_obstacles_per_point, obstacle_sample_step, footprint);
  }
}

void BsplineReplanner::setRcEsdfObstacles(
    const std::vector<Eigen::Vector3d>& obstacles) {
  if (traj_optimizer) {
    traj_optimizer->setRcEsdfObstacles(obstacles);
  }
}
