...
void GradBandOptimizer::setParameterAuto(double max_v, double max_a) {
  ...
  rc_esdf_lambda = 0.0;
  rc_esdf_margin = 0.05;
  rc_esdf_query_radius = 1.2;
  rc_esdf_obstacle_sample_step = 1;
  rc_esdf_max_obstacles_per_point = 120;
  rc_esdf_enabled = false;
  rc_esdf_ready = false;
  alpha = 0.002;
  beta = 0.01; 
  algorithm = 11;
...
void GradBandOptimizer::setEsdfPenalty(double distance_weight, double safe_distance) {
  lamda2 = std::max(0.0, distance_weight);
  dist0 = std::max(0.0, safe_distance);
}

void GradBandOptimizer::setRcEsdfConfig(
  ...
  rc_esdf_enabled = enabled;
  rc_esdf_ready = false;
  rc_esdf_lambda = std::max(0.0, lambda);
  rc_esdf_margin = std::max(0.0, margin);
  rc_esdf_query_radius = std::max(0.0, query_radius);
  rc_esdf_max_obstacles_per_point = std::max(0, max_obstacles_per_point);
  rc_esdf_obstacle_sample_step = std::max(1, obstacle_sample_step);

  if (!rc_esdf_enabled || rc_esdf_lambda <= 0.0 || footprint.size() < 3 ||
      map_width <= 0.0 || map_height <= 0.0 || resolution <= 0.0) {
    rc_esdf_enabled = false;
    return;
  }

  rc_esdf_map.initialize(map_width, map_height, resolution);
  rc_esdf_map.generateFromPolygon(footprint);
  rc_esdf_ready = true;
}

void GradBandOptimizer::setRcEsdfObstacles(
    const std::vector<Eigen::Vector3d>& obstacles) {
  rc_esdf_obstacles.clear();
  rc_esdf_obstacles.reserve(obstacles.size());
  for (const auto& obstacle : obstacles) {
    rc_esdf_obstacles.emplace_back(obstacle.x(), obstacle.y());
  }
}
...
double GradBandOptimizer::getRcEsdfCostAndGradient(
    const Eigen::Vector3d& q1, const Eigen::Vector3d& q2,
    const Eigen::Vector3d& q3, Eigen::Vector3d& grad_q2) const {
  grad_q2.setZero();
  if (!rc_esdf_enabled || !rc_esdf_ready || rc_esdf_obstacles.empty()) {
    return 0.0;
  }

  Eigen::Vector2d tangent(q3.x() - q1.x(), q3.y() - q1.y());
  ...
  const Eigen::Vector2d center(q2.x(), q2.y());
  const double query_radius_sq = rc_esdf_query_radius > 0.0
      ? rc_esdf_query_radius * rc_esdf_query_radius
      : std::numeric_limits<double>::infinity();

  double cost = 0.0;
  int used_obstacles = 0;
  for (size_t obs_id = 0; obs_id < rc_esdf_obstacles.size();
       obs_id += static_cast<size_t>(rc_esdf_obstacle_sample_step)) {
    const Eigen::Vector2d diff_world = rc_esdf_obstacles[obs_id] - center;
    if (diff_world.squaredNorm() > query_radius_sq) {
      continue;
    }

    const Eigen::Vector2d obstacle_body = world_to_body * diff_world;
    double dist = 0.0;
    Eigen::Vector2d grad_body = Eigen::Vector2d::Zero();
    if (!rc_esdf_map.query(obstacle_body, dist, grad_body)) {
      continue;
    }

    if (dist < rc_esdf_margin) {
      const double violation = rc_esdf_margin - dist;
      cost += violation * violation;

      const Eigen::Vector2d grad_world = 2.0 * violation * body_to_world * grad_body;
      grad_q2.x() += grad_world.x();
      grad_q2.y() += grad_world.y();
    }

    ++used_obstacles;
    if (rc_esdf_max_obstacles_per_point > 0 &&
        used_obstacles >= rc_esdf_max_obstacles_per_point) {
      break;
    }
  }
...
double GradBandOptimizer::costFuncMinCurvaturePara(const std::vector<double>& x,
  ...
  for (int i = 0; i < opt->control_points.rows(); ++i) {
    ...
    Eigen::Vector3d g_rc_q2 = Eigen::Vector3d::Zero();
    f_rc_esdf += opt->getRcEsdfCostAndGradient(q1, q2, q3, g_rc_q2);
