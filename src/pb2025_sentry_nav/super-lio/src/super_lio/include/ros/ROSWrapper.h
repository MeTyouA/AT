...
public:
  ...
  void pub_pointlio_compat_odom(const NavState& state);
  void pub_pointlio_compat_cloud(const BASIC::CloudPtr& pc_world, double time);

  void set_global_map(const BASIC::CloudPtr& global_map);
  void set_pointlio_compat_origin(const BASIC::SE3& initial_lidar_pose_world);

  void set_initial_data(BASIC::SE3& init_pose, bool& flg_get_init_guess, bool flg_finish_init = false);
  bool pointlio_compat_enabled() const { return pointlio_compat_enable_; }
...
private:
  ...
  bool pointlio_compat_enable_ = true;
  bool pointlio_compat_origin_initialized_ = false;
  BASIC::SE3 pointlio_compat_world_to_lidar_odom_;
...
private:
  ...
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_pointlio_compat_odom_;
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_pointlio_compat_cloud_;
};
