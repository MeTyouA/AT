...
bool SuperLIO::kf_init(){
  ...
  V3 init_gyro_bias = mean_gyro;
  if (g_kf_init_gyro_bias_max_norm >= 0.0 &&
      mean_gyro.norm() > g_kf_init_gyro_bias_max_norm) {
    LOG(WARNING) << YELLOW << " ---> [Init]: mean gyro " << mean_gyro.transpose()
                 << " exceeds bias threshold " << g_kf_init_gyro_bias_max_norm
                 << ", use zero gyro bias." << RESET;
    init_gyro_bias = V3::Zero();
  }
  kf_->SetInitialConditions(options, init_gyro_bias, V3::Zero(), imu_scale, ref_gravity);
...
void SuperLIO::Output(){
  ...
  if(g_visual_map){
    ...
    count = 0;
    if(g_visual_dense){
      CloudPtr src = scan_undistort_full_;
      if (g_pub_voxel_size > 0.0) {
        static pcl::VoxelGrid<PointType> pub_voxel;
        pub_voxel.setLeafSize(g_pub_voxel_size, g_pub_voxel_size, g_pub_voxel_size);
        pub_voxel.setInputCloud(scan_undistort_full_);
        CloudPtr down(new PointCloudType());
        pub_voxel.filter(*down);
        src = down;
      }
      pcl::transformPointCloud(*src, *world_pc, transformation);
