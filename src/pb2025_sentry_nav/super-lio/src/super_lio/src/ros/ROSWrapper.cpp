...
void LoadParamFromRos(rclcpp::Node& node)
{
  ...
  node.declare_parameter<double>("lio.kf.init_gyro_bias_max_norm", -1.0);
  node.get_parameter("lio.kf.init_gyro_bias_max_norm", g_kf_init_gyro_bias_max_norm);
  ...
  node.declare_parameter<double>("lio.output.pub_voxel_size", 0.0);
  node.get_parameter("lio.output.pub_voxel_size", g_pub_voxel_size);
  ...
  for(std::size_t _i = 0; _i < ptsize; _i += g_filter_rate){
    ...
    if (tag == 0x10 || tag == 0x00){
      ...
      if(dis > g_blind2 && dis < g_maxrange2){
        const double point_offset_time = pt.offset_time * 1e-9 * g_point_time_scale;
        offset_time = std::max(offset_time, point_offset_time);
        lidar_data.pc->emplace_back(pt.x, pt.y, pt.z, pt.reflectivity, point_offset_time);
      }
    }
  }
...
void ROSWrapper::stdMsgHandler(const sensor_msgs::msg::PointCloud2::SharedPtr msg){
  ...
  case LID_TYPE::HESAI16:
  {
    ...
    for(std::size_t i = 0; i < pl_orig.size(); i += g_filter_rate)
    {
      ...
      const double point_offset_time = (pt.timestamp - time_begin) * g_point_time_scale;
      offset_time = std::max(offset_time, point_offset_time);
  ...
  case LID_TYPE::VEL_NCLT:
  {
    ...
    for(std::size_t i = 0; i < pl_orig.size(); i += g_filter_rate){
      auto& pt = pl_orig.points[i];
      if (!validPoint(pt.x, pt.y, pt.z)) continue;
      const double point_offset_time = pt.time * 1e-6 * g_point_time_scale;
      offset_time = std::max(offset_time, point_offset_time);
      lidar_data.pc->emplace_back(
          pt.x, pt.y, pt.z, 1.0, point_offset_time);
    }
    lidar_data.end_time = lidar_data.start_time + offset_time;
    break;
  }
  ...
  case OUSTER:
  {
    ...
    for(std::size_t i = 0; i < pl_orig.size(); i += g_filter_rate){
      auto& pt = pl_orig.points[i];
      if (!validPoint(pt.x, pt.y, pt.z)) continue;
      const double point_offset_time = pt.t * 1e-9 * g_point_time_scale;
      offset_time = std::max(offset_time, point_offset_time);
      lidar_data.pc->emplace_back(
          pt.x, pt.y, pt.z, pt.intensity, point_offset_time);
    }
