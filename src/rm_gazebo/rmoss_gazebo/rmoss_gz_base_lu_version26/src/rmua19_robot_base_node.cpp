...
#include "rmoss_gz_base_lu_version26/rmua19_robot_base_node.hpp"
#include <memory>
#include <string>
#include <vector>
...
Rmua19RobotBaseNode::Rmua19RobotBaseNode(const rclcpp::NodeOptions& options)
{
    ...
    // AT_joint 单头哨兵 yaw
    std::string gz_gimbal_yaw_cmd_topic = "/model/" + robot_name + "/joint/gimbal_big_to_gimbal_yaw_joint/cmd_vel";

    // AT_joint 单头哨兵 gimbal_big  一个大yaw
    std::string gz_gimbal_big_cmd_topic = "/model/" + robot_name + "/joint/chassis_to_gimbal_big_joint/cmd_vel";
    // AT_joint 单头哨兵 picth
    std::string gz_gimbal_pitch_cmd_topic = "/model/" + robot_name + "/joint/gimbal_yaw_to_gimbal_pitch_joint/cmd_vel";
    ...
    std::string gz_gimbal_pitch_imu_topic = "/world/" + world_name + "/model/" + robot_name + "/link/gimbal_pitch/sensor/gimbal_pitch_imu/imu";
    ...
    gimbal_yaw_vel_actuator_ = std::make_shared<rmoss_gz_base::IgnGimbalActuator>(
        node_, gz_node_, gz_gimbal_yaw_cmd_topic, gz_gimbal_pitch_cmd_topic);
    ...
    gz_gimbal_yaw_encoder_ = std::make_shared<rmoss_gz_base::IgnGimbalEncoder>(
        node_, gz_node_, gz_joint_state_topic, std::vector<std::string> { "gimbal_big_to_gimbal_yaw_joint" },
        std::vector<std::string> { "gimbal_yaw_to_gimbal_pitch_joint" });
    gz_gimbal_big_imu_ = std::make_shared<rmoss_gz_base::IgnGimbalImu>(
        node_, gz_node_, gz_gimbal_big_imu_topic);
    gz_gimbal_pitch_imu_ = std::make_shared<rmoss_gz_base::IgnGimbalImu>(
        node_, gz_node_, gz_gimbal_pitch_imu_topic);
    ...
    gimbal_yaw_controller_ = std::make_shared<rmoss_gz_base::GimbalController>(
        node_, gimbal_yaw_vel_actuator_, gz_gimbal_pitch_imu_->get_position_sensor(), "gimbal_yaw", "gimbal_big_to_gimbal_yaw_joint", "gimbal_yaw_to_gimbal_pitch_joint");
    ...
    gimbal_yaw_vel_actuator_->enable(true);
    shoot_actuator_->enable(true);
    gz_gimbal_big_encoder_->enable(true);
    gz_gimbal_yaw_encoder_->enable(true);
    gz_gimbal_big_imu_->enable(true);
    gz_gimbal_pitch_imu_->enable(true);
    if (use_odometry) {
...
void Rmua19RobotBaseNode::enable_power_cb(const std_msgs::msg::Bool::SharedPtr msg)
{
    if (msg->data) {
        ...
        gimbal_yaw_vel_actuator_->enable(false);
